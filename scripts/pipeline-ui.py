...
def grab_clip(song: dict):
    ...
    rc = run_command([
        ...
        song["url"], song["slug"], song["start"], str(song["duration"]),
    ])
    if rc != 0:
...
def run_stems(song: dict, force: bool = False):
    ...
    if not clip_path.exists():
        console.print("[red]✗ no clip — grab it first[/red]")
        return
    console.print(f"\n[#C9A96E]▸[/#C9A96E] running stems: [bold]{song['name']}[/bold]")
    cmd = [str(REPO_ROOT / "scripts" / "run-stems.sh"), str(clip_path), song["slug"]]
    if force:
        cmd.append("--force")
    rc = run_command(cmd, env={"AURIS_DEVICE": os.environ.get("AURIS_DEVICE", "auto")})
    if rc != 0:
...
def run_analyse(song: dict):
    ...
    has_npy = stems_dir.exists() and any(stems_dir.glob("*.npy"))
    if not has_npy:
        console.print("[red]✗ no stem .npy files — run stems first[/red]")
        return
    console.print(f"\n[#C9A96E]▸[/#C9A96E] analysing: [bold]{song['name']}[/bold]")
    rc = run_command([str(REPO_ROOT / "scripts" / "run-analyse.sh"), song["slug"]])
    if rc != 0:
...
def export_curves(song: dict):
    ...
    if not curves_dir.exists():
        console.print("[red]✗ no curves — run analyse first[/red]")
        return
    console.print(f"\n[#C9A96E]▸[/#C9A96E] exporting all curves to docs/proto/data/")
    rc = run_command([
    ...
    while True:
        ...
        if action == "q":
            ...
            if new_song:
                ...
                save_songs(songs)
                console.print("[#A084C8]✓ added and saved[/#A084C8]")

        elif action == "r":
            console.print(f"\n[#C9A96E]▸[/#C9A96E] running all {len(songs)} songs\n")
            for i, song in enumerate(songs, 1):
            ...
            console.print(f"\n[#C9A96E]▸[/#C9A96E] re-analysing all songs with stems\n")
            rc = run_command([str(REPO_ROOT / "scripts" / "run-analyse.sh")])
            if rc != 0:
                console.print("[red]✗ analysis failed[/red]")
            else:
                console.print("[#A084C8]✓ all curves updated[/#A084C8]")
